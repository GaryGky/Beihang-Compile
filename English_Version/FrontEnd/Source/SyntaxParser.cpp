...
SyntaxParser::SyntaxParser(LexicalParser &lex, SymbolTabel &symbolTabel1) :
    ...
    varCount = 0;
    stringCount = 0;
    condRelation = BGE; // 随便设置一个，在用时总会先赋值再使用
    varScope = "Global"; // 初始化为全局作用域
    order = 0;  // 初始化序号为0
    globalRegCnt = 0;
    // haveReturn = false; // 函数进入之前也会设置为false 表示不含return语句
    relationMap.insert(pair<string, TmpCodeType>("!=", BNE));
    relationMap.insert(pair<string, TmpCodeType>("==", BEQ));
    relationMap.insert(pair<string, TmpCodeType>(">=", BGE));
    relationMap.insert(pair<string, TmpCodeType>("<=", BLE));
    relationMap.insert(pair<string, TmpCodeType>(">", BGT));
    relationMap.insert(pair<string, TmpCodeType>("<", BLT));
}
    ...
    while (true) {
        ...
        if (c == '+' || c == '-') {
            ...
            if (isdigit(c)) {
                ...
                localSymbolSpace.insert(pair<string, SymbolItem>(iden, item)); // 加入局部变量/常量表
            }
            ...
            SymbolItem item(Constant, iden, level, Integer, token, order, varScope, tmp); // 新建一个符号项
            symbolTabel.pushSymTabel(item); // 将符号压入符号表
            localSymbolSpace.insert(pair<string, SymbolItem>(iden, item));
            fourComExpr.left = token; // 无符号数字
        }
        fourComExpr.varScope = varScope; // 定义了四元式作用域
...
void SyntaxParser::constCharDef() {
    ...
    while (true) {
        ...
        localSymbolSpace.insert(pair<string, SymbolItem>(iden, item)); // 加入局部表
        if (lexicalParser.peekTop() == ';') {
            break;
        }
        token = lexicalParser.getWord();  // , 逗号
    ...
    while (true) {
        ...
        if (c == ',' || c == ';') {
            if (type == "int") {
                ...
                localSymbolSpace.insert(pair<string, SymbolItem>(token, item));
            } else if (type == "char") {
            ...
            if (type == "int") {
                ...
                SymbolItem item(Variable, fourComExpr.target, level,
                                Integer, value, order, varScope, tmp);
                symbolTabel.pushSymTabel(item);
                localSymbolSpace.insert(pair<string, SymbolItem>(fourComExpr.target, item));
            } else if (type == "char") {
                fourComExpr.valueType = Character; // 设置中间码的类型
                SymbolItem item(Variable, fourComExpr.target, level,
                                Character, value, order, varScope, tmp);
...
void SyntaxParser::funcState() {
    ...
    while (true) {
        ...
        localSymbolSpace.clear(); // 清空上一个函数的局部变量空间
...
void SyntaxParser::condParse(const string &labelA) {
    char c;
    string token;
    ...
    if (c != '!' && c != '<' && c != '>' && c != '=') {
        ...
        // 说明只有单个表达式
        left = exprLeft.name;
        right = "0"; // 单个表达式的话右边默认为0，并且是大于号
        condRelation = relationMap.find(">")->second; // 大于0条件成立
    } else {
        token = lexicalParser.getWord(); // 关系运算符
        condRelation = relationMap.find(token)->second; //将关系运算符赋值
        exprRight = exprParse(); // 右表达式解析
        // mips的比较
        left = exprLeft.name; // 只能是寄存器 :: 后端如果不是寄存器则要存到寄存器中
        right = exprRight.name; // 默认为0
    }
    switch (condRelation) {
        // 不满足if条件时时才会跳转
        // cnodRelation :: 需要满足的条件
        case BEQ:  /// ==
            fourComExpr.type = BNE; // 不等于0跳转
            fourComExpr.left = left; // 左边为第一个比较数
            fourComExpr.index1 = right; // 右边是第二个比较数
            fourComExpr.target = labelA; // 跳到else语句
            fourComExpr.varScope = varScope;
            fourComExpr.right = "bne";
            tmpCodeVector.push_back(fourComExpr);
            break;
        case BNE: // !=
            fourComExpr.type = BEQ; // 等于0时跳转
            fourComExpr.left = left;
            fourComExpr.index1 = right; // 右边是第二个比较数
            fourComExpr.target = labelA;
            fourComExpr.varScope = varScope;
            fourComExpr.right = "beq";
            tmpCodeVector.push_back(fourComExpr);
            break;
        case BLE:  // <=
            fourComExpr.type = BGT; // 大于0时跳转
            fourComExpr.left = left;
            fourComExpr.index1 = right; // 右边是第二个比较数
            fourComExpr.target = labelA;
            fourComExpr.varScope = varScope;
            fourComExpr.right = "bgt";
            tmpCodeVector.push_back(fourComExpr);
            break;
        case BLT: // <
            fourComExpr.type = BGE; // 大于等于0 时跳转
            fourComExpr.left = left;
            fourComExpr.index1 = right; // 右边是第二个比较数
            fourComExpr.target = labelA;
            fourComExpr.varScope = varScope;
            fourComExpr.right = "bge";
            tmpCodeVector.push_back(fourComExpr);
            break;
        case BGT: // >
            fourComExpr.type = BLE; // 小于等于0时跳转
            fourComExpr.left = left;
            fourComExpr.index1 = right; // 右边是第二个比较数
            fourComExpr.target = labelA;
            fourComExpr.varScope = varScope;
            fourComExpr.right = "ble";
            tmpCodeVector.push_back(fourComExpr);
            break;
        case BGE: // >=
            fourComExpr.type = BLT; // 小于0时跳转
            fourComExpr.left = left;
            fourComExpr.index1 = right; // 右边是第二个比较数
            fourComExpr.target = labelA;
            fourComExpr.varScope = varScope;
            fourComExpr.right = "blt";
            tmpCodeVector.push_back(fourComExpr);
            break;
        default:
            break;
...
void SyntaxParser::ifParse(RetType retType) {
    ...
    fourComExpr.varScope = varScope;
    token = lexicalParser.getWord(); // (
    condParse(labelA); // 条件分析 -- 并返回比较的结果
...
void SyntaxParser::forParse(RetType retType) {
    ...
    fourComExpr.type = AssignState;
    fourComExpr.right = fourComExpr.target;
...
void SyntaxParser::scanfParse() {
    ...
    while (true) {
        ...
        token = lexicalParser.getWord(); // 标识符
        RetType varType;
        if (localSymbolSpace.find(token) != localSymbolSpace.end()) {
            varType = localSymbolSpace.find(token)->second.getRetType(); // 局部优先原则
        } else {
...
RetType SyntaxParser::returnParse() {
    ...
    if (c == ';') {
        ...
        if (expRet.name.find('[') != string::npos) {
            // 要把下标和名称分开
...
int SyntaxParser::getConstFacValue(ExprCompose exprCompose) {
    if (isConstFactor(exprCompose) == IntCompose) {
        return stoi(exprCompose.target);
    } else if (isConstFactor(exprCompose) == CharCompose) {
        return (exprCompose.target[1]); // 字符
    } else if (isConstFactor(exprCompose) == ConstComposeGlobal) {
        if (exprCompose.retType == Integer) {
            return symbolTabel.getIntValueByName(exprCompose.target);
        } else {
            return symbolTabel.getCharValueByName(exprCompose.target);
        }
    } else if (isConstFactor(exprCompose) == ConstComposeLocal) {
        if (exprCompose.retType == Integer) {
            return stoi(localSymbolSpace.find(exprCompose.target)->second.getValue(), nullptr, 10);
        } else {
            return localSymbolSpace.find(exprCompose.target)->second.getValue()[0]; // 一个字符
        }
    }
    return -214789100; // 返回错误编码
}

string SyntaxParser::calConstExprCom(int a, int b, const string &op) {
    if (op == "+") {
...
string SyntaxParser::revPolCal(vector<ExprCompose> &revPolExpr) {
    ...
    for (auto &it : revPolExpr) {
        if (!it.isOperator) {
            ...
            if (isConstFactor(exprCompose1) != NullCompose && isConstFactor(exprCompose2) != NullCompose) {
                // 合并成新的exprCompose加入栈中然后continue
                int newValue1, newValue2;
                newValue1 = getConstFacValue(exprCompose1); // 运算数1
                newValue2 = getConstFacValue(exprCompose2); // 运算数2
                exprComposeNew.isOperator = false;
                exprComposeNew.target = calConstExprCom(newValue2, newValue1, it.op);
                exprComposeNew.retType = Integer;
                calResStack.push(exprComposeNew);
                continue;
            }
            // 储存中间结果
            ...
            // 如果是数组的话需要设置数组下标
            if (exprCompose2.target.find('[') != string::npos) {
            ...
            if (exprCompose1.target.find('[') != string::npos) {
...
ExprComType SyntaxParser::isConstFactor(ExprCompose exprCompose) {
    // 数字 字符 常量
    if (isdigit(exprCompose.target[0]) || exprCompose.target[0] == '+' || exprCompose.target[0] == '-') {
        return IntCompose;
    } else if (exprCompose.target[0] == '\'') {
        return CharCompose;
    } else if (localSymbolSpace.find(exprCompose.target) != localSymbolSpace.end() &&
               localSymbolSpace.find(exprCompose.target)->second.getItemType() == Constant) {
        return ConstComposeLocal;
    } else if (symbolTabel.getItemtypeByName(exprCompose.target) == Constant) {
        return ConstComposeGlobal;
    } else {
        return NullCompose;
    }
}
...
ExprCompose SyntaxParser::factorParse() {
    ...
    if (c == '(') {
        ...
        if (localSymbolSpace.find(idenName) != localSymbolSpace.end()) {
            exprCompose.retType = localSymbolSpace.find(idenName)->second.getRetType(); // 从局部空间中查找
        } else {
...
void SyntaxParser::mainParse() {
    ...
    localSymbolSpace.clear(); // 将之前的局部变量空间清除
    order = 0; // 一个函数定义完之后order需要清零 -- 记录函数局部变量的序号
    ...
    for (auto &it : tmpCodeVector) {
        switch (it.type) {
            ...
            case BEQ: // 等于
                ...
                output.append(it.index1 + ", "); // 比较的第二个操作数
                output.append(it.target); // Label
                tmpCode << output << endl;
                break;
            case BNE: // 不等于
