...
#include "../../BackEnd/Header/FourComExpr.h"
#include <queue>
#include <string>
#include <set>
#include <map>
...
private:
    ...
    map<string, SymbolItem> localSymbolSpace; // 函数局部变量和常量的空间
...
public:
    ...
    // 查看这个表达式组成在后缀计算中是否可以化简
    ExprComType isConstFactor(ExprCompose exprCompose); // 数字，字母，常量
    int getConstFacValue(ExprCompose exprCompose);

    string calConstExprCom(int a, int b, const string &op); // 计算表达式的常量值
    ...
    void condParse(const string &labelA); // 条件解析
