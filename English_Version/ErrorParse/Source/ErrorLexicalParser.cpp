...
ErrorLexicalParser::ErrorLexicalParser(ErrorParse &errorParse1) : errorParse(errorParse1) {
}

void ErrorLexicalParser::start() {
    initial();
    setWordMap();
    setOpMap();
    setSinDiv();
...
void ErrorLexicalParser::initial() {
...
void ErrorLexicalParser::output(const string &token) {
    if (!outputSwitch) {
        return;
    }
    string res1;
...
bool ErrorLexicalParser::isSingleDiv(const string &c) {
    return singleDiv.find(c) != singleDiv.end();
}

bool ErrorLexicalParser::isDoubleDiv(char c) {
    return (c == '<' || c == '>' || c == '=');
}

void ErrorLexicalParser::trim() {
...
void ErrorLexicalParser::toNextSemi() {
    while (filepos != filelength) {
...
bool ErrorLexicalParser::isAlpha(char c) {
    return isalpha(c) || c == '_';
}

char ErrorLexicalParser::peekTop(int n) {
    long tmp1 = lineNo;
    long tmp2 = filepos;
...
void ErrorLexicalParser::setOpMap() {
...
void ErrorLexicalParser::setSinDiv() {
    singleDiv.insert("+");
    singleDiv.insert("-");
    singleDiv.insert("*");
    singleDiv.insert("/");
...
string ErrorLexicalParser::queryWord(const string &token) {
...
string ErrorLexicalParser::queryOp(const string &token) {
...
void ErrorLexicalParser::setWordMap() {
...
bool ErrorLexicalParser::isChar(char c) {
    return isAlpha(file[filepos]) || isdigit(file[filepos]);
}

string ErrorLexicalParser::getWord(bool on_off) {
    ...
    if (c == '\'') {
        // 字符
        c = file[filepos++];
        if (!isAlpha(c) && !isalnum(c) && c != '+' && c != '-' && c != '*' && c != '/') {
            output(to_string(lineNo) + 'a'); // 如果字符常量非法
        }
        token += c;
        c = file[filepos++]; // 吃掉单引号
        if (c != '\'') {
            errorParse.syntaxParseError(lineNo, LexError); // 如果不是单引号，则报错
        }
        ...
        if (c == '\"') {
            ...
            // 缺少双引号，词法分析出错处理
            errorParse.syntaxParseError(lineNo, LexError); // 字符串缺少右引号
        }
        ...
        if (isSingleDiv(token)) {
            ...
            if (file[filepos] == '=') {
                ...
                errorParse.syntaxParseError(lineNo, LexError); // 不等号后面不是等号 !=
            }
        } else {
            errorParse.syntaxParseError(lineNo, LexError);
        }
...
void ErrorLexicalParser::retract() {
    lineNo = lastLineNo;
    filepos = lastFilePos; // 退回到上一个单词的位置
}

long ErrorLexicalParser::getLineNo() {
    return lineNo;
}

void ErrorLexicalParser::setLineNo(long lineno) {
    lineNo = lineno;
}

void ErrorLexicalParser::toNextChar(char chDst) {
    while (filepos != filelength) {
