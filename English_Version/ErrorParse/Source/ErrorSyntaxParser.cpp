...
#include "../Header/ErrorSyntaxParser.h"

#include <utility>
#include <iostream>


ErrorSyntaxParser::ErrorSyntaxParser(ErrorLexicalParser &lex, ErrorParse &parse, ErrorSymbolTabel &symbolTabel1)
        : lexicalParser(lex), errorParse(parse), globalSymbolTable(symbolTabel1) {
    level = 0; // 初始化为0层 最外层的全局变量
    curOwnsRet = false;
    // varScope = "Global";
}

void ErrorSyntaxParser::start() {
    constState();
    varState();
    funcState();
    mainParse();
...
void ErrorSyntaxParser::constState() {
    // 常量说明
    if (!isConstState()) {
        return;
    }
...
void ErrorSyntaxParser::constDef() {
    string token;
...
void ErrorSyntaxParser::constIntDef() {
    vector<ErrorSymbolItem> tmp; // 构造一个空引用放到符号表中占位置
    string token;
    string iden;
    while (true) {
        ...
        if (c == '+' || c == '-') {
            ...
            if (isdigit(c)) {
                ...
                ErrorSymbolItem item(Constant, iden, level, Integer, token, tmp); // 新建一个符号项
                globalSymbolTable.pushSymTabel(item); // 将符号压入符号表
            } else {
                // 考虑到只有一个加号的错误
...
void ErrorSyntaxParser::constCharDef() {
    string token;
    string iden;
    vector<ErrorSymbolItem> tmp;
    while (true) {
        iden = lexicalParser.getWord(); // 标识符
        token = lexicalParser.getWord(); // =
        // 提前判断三个字符
        ...
        if (char0 == '\'' && char2 == '\'' && isConstChar(char1)) {
            ...
            ErrorSymbolItem item(Constant, iden, level, Character, token, tmp); // 新建一个符号项
            globalSymbolTable.pushSymTabel(item); // 将符号压入符号表, 填表前自动查表
...
void ErrorSyntaxParser::varState() {
    if (!isVarState()) {
        return;
    }
    string token;
...
void ErrorSyntaxParser::varDef() {
    ...
    while (true) {
        ...
        long tmpLine = lineNo;
        if (type == "int") {
            ErrorSymbolItem item(Variable, token, level, Integer, value, tmp);
            globalSymbolTable.pushSymTabel(item);
        } else if (type == "char") {
            ErrorSymbolItem item(Variable, token, level, Character, value, tmp);
            globalSymbolTable.pushSymTabel(item);
        }
        c = lexicalParser.peekTop();
        if (c == ',') {
...
void ErrorSyntaxParser::funcState() {
    // 函数说明
    string token;
    string type;
    string name;
    RetType retType = Void; // 默认为void
    vector<ErrorSymbolItem> paraVector; // 解析参数表得到的参数
    while (true) {
        // 不断的定义函数
        if (isMain()) {
            break;
        ...
        if (globalSymbolTable.checkIdenDef(name) != NullType) {
            // 函数名称已经定义过
        ...
        if (type == "int") {
            ...
            ErrorSymbolItem item(Function, name, level, Integer, value1, paraVector);
            globalSymbolTable.pushSymTabel(item);
        } else if (type == "char") {
            retType = Character;
            value1 = to_string(paraVector.size());
            ErrorSymbolItem item(Function, name, level, Character, value1, paraVector);
            globalSymbolTable.pushSymTabel(item);
        } else {
            value1 = to_string(paraVector.size());
            retType = Void;
            ErrorSymbolItem item(Function, name, level, Void, value1, paraVector);
            globalSymbolTable.pushSymTabel(item);
        }
        for (const auto &it : paraVector) {
            globalSymbolTable.pushSymTabel(it);
        }
        level++; // level 变成1
        token = lexicalParser.getWord(); // {
        ...
        token = lexicalParser.getWord(); // }
        globalSymbolTable.dropSymTabel(); // 清空这层函数的局部变量
        level--;
        if (type == "void") {
...
vector<ErrorSymbolItem> ErrorSyntaxParser::paraParse() {
    string token;
    string type;
    vector<ErrorSymbolItem> paraVector;
    char c;
    while (true) {
        c = lexicalParser.peekTop();
        if (c == '{') {
        ...
        if (type == "int") {
            ErrorSymbolItem item(Parament, token, level, Integer, value, tmp);
            paraVector.push_back(item);
        } else if (type == "char") {
            ErrorSymbolItem item(Parament, token, level, Character, value, tmp);
            paraVector.push_back(item);
        } else {
...
bool ErrorSyntaxParser::stateColParse(RetType retType) {
    string token;
    bool ownsRet = false;
    char c = lexicalParser.peekTop();
    while (true) {
...
void ErrorSyntaxParser::stateParse(RetType retType) {
    string token;
    token = lexicalParser.getWord();
    if (token == "if") {
        // IF
        ...
        // 赋值语句不能在函数表中找到标识符
        RetType ridenDef = globalSymbolTable.checkIdenDef(token); // 检查标识符是否定义
        ...
        if (lexicalParser.isVoidFunc(token)) {
            ...
            if (ridenDef != NullType) {
                // 如果标识符未定义则跳过查表
                ErrorSymbolItem assignObj = globalSymbolTable.getSymbol(token);
...
void ErrorSyntaxParser::condParse() {
    char c;
    string token;
    bool isIntExpr;
    isIntExpr = exprParse();
...
void ErrorSyntaxParser::ifParse(RetType retType) {
    string token;
    char c;
    token = lexicalParser.getWord(); // (
    condParse();
...
void ErrorSyntaxParser::whileParse(RetType retType) {
    string token;
    token = lexicalParser.getWord(); // (
    condParse();
    char c = lexicalParser.peekTop();
...
void ErrorSyntaxParser::forParse(RetType retType) {
    // retType 表示当前所在函数的类型，为return语句服务的
    string token;
    RetType idenRetType; // 标识符的返回值
    token = lexicalParser.getWord(); // (
    token = lexicalParser.getWord(); // 标识符
    idenRetType = globalSymbolTable.checkIdenDef(token); // 检查标识符是否定义
...
void ErrorSyntaxParser::doWhileParse(RetType retType) {
    stateParse(retType);
    string token;
...
void ErrorSyntaxParser::scanfParse() {
    string token;
    RetType retType;
    token = lexicalParser.getWord(); // (
    while (true) {
        token = lexicalParser.getWord(); // 标识符
        retType = globalSymbolTable.checkIdenDef(token); // 检查标识符是否定义
        if (retType == NullType) {
            errorParse.semanticError(lineNo, Nodeclare);
        } // 检查标识符是否定义
        ErrorSymbolItem it = globalSymbolTable.getSymbol(token);
...
void ErrorSyntaxParser::printfParse() {
    string token;
    char c;
    token = lexicalParser.getWord(); // (
    c = lexicalParser.peekTop();
...
RetType ErrorSyntaxParser::returnParse() {
    string token;
    char c;
    c = lexicalParser.peekTop();
    if (c == ';') {
...
void ErrorSyntaxParser::assignParse() {
    char c;
    string token;
    c = lexicalParser.peekTop();
    if (c == '[') {
...
void ErrorSyntaxParser::funcRefParse(int flag, string funcName) {
    // flag = -1: 表示函数名称未定义
    string token;
    token = lexicalParser.getWord(); // (
    if (flag != -1) {
...
void ErrorSyntaxParser::excValueTabParse() {
    // 异常参数表解析
    string token;
...
void ErrorSyntaxParser::valueTabParse(string funcName) {
    string token;
    int paraNum = 0; // 记录参数的个数
    bool isIntExpr; // 表示是否为整型表达式
    vector<ErrorSymbolItem> paraVector = globalSymbolTable.getFuncPara(funcName); // 获取该函数的参数表
    char c = lexicalParser.peekTop();
    if (c == ')') {
        // 值参数表为空
        lexicalParser.output("<值参数表>");
...
bool ErrorSyntaxParser::exprParse() {
...
bool ErrorSyntaxParser::termParse() {
    char c;
...
bool ErrorSyntaxParser::factorParse() {
    char c;
    string token;
    ...
    if (c == '(') {
        ...
        token = lexicalParser.getWord(); // 标识符
        // 查符号表：返回该标识符的类型
        RetType retType = globalSymbolTable.checkIdenDef(token); // 检查标识符是否定义
        if (retType == NullType) {
...
void ErrorSyntaxParser::mainParse() {
    string token;
    level++; // main里面的层数也要加一
...
void ErrorSyntaxParser::comStateParse(RetType retType) {
    constState(); // 常量说明
    varState(); // 变量说明
...
bool ErrorSyntaxParser::isVarState() {
...
bool ErrorSyntaxParser::isConstState() {
    long record = lexicalParser.getFilepos();
    long lastLineNo = lexicalParser.getLineNo();
    string token;
