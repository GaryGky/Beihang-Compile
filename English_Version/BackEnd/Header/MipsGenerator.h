...
private:
    ...
    // map<string, string> varToGlobalReg; // 临时变量对应到全局寄存器
    map<string, set<string>> funcToDef; // 函数到其内部定义的变量
    map<string, string> regToPos; // 寄存器映射到对应栈的位置
    map<string, set<string>> recFuncToDefstate; // 递归函数到递归函数的参数
public:
    explicit MipsGenerator(vector<FourComExpr> &tmpCode, SymbolTabel &symbolTabel1);

    void genMipsCode(); // 产生mips代码

    void genFuncDefReg(); // 产生一个函数内部改变的寄存器
    void genRecFunc(); // 产生递归函数
    ...
    void keepScene(set<string> &funcDefSet, string &targetFunc); // 保存现场
    void restoreScene(set<string> &funcDefReg, string &function); // 恢复现场
    ...
    string getRecurPara(int paraOrder, const string &dstReg); // 汉诺塔问题中的参数
};
