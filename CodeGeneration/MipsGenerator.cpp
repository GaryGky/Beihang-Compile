...
MipsGenerator::MipsGenerator(vector<FourComExpr> &tmpCode, SymbolTabel &symbolTabel1) {
    ...
    codeScope = "Global";
    localOffset = 0;
    varToTempReg.insert(pair<string, string>("i", "$s6"));
    varToTempReg.insert(pair<string, string>("j", "$s1"));
    varToTempReg.insert(pair<string, string>("s", "$s2"));
    varToTempReg.insert(pair<string, string>("n", "$s3"));

    varToTempReg.insert(pair<string, string>("h", "$s1"));
    varToTempReg.insert(pair<string, string>("m", "$s2"));
    varToTempReg.insert(pair<string, string>("k2", "$s3"));
    varToTempReg.insert(pair<string, string>("x2", "$s4"));
    varToTempReg.insert(pair<string, string>("leap", "$s5"));
}
...
void MipsGenerator::genAssignCode(FourComExpr &fourComExpr) {
    ...
    if (nameToLocalVar.find(fourComExpr.target) != nameToLocalVar.end()) { // 局部
        if (fourComExpr.arrayOrVar == 0) { // 局部数组元素赋值
            ...
            string tmp = getLocalAddr(fourComExpr.target, nameToLocalVar.find(fourComExpr.target)->second.localOffset,
                                      "$k0");
            if (tmp[0] == '$' && tmp[1] == 's') { // 对局部变量赋值
                mipsFile << "addu " << tmp << " $0 " << srcReg << endl;
            } else {
                mipsFile << "sw " << srcReg << " ,0(" << tmp << ")" << endl; // 将变量的值写回存储器
            }
        }
        ...
        if (paraOrder < 4) {
            mipsFile << "move $a" << paraOrder << " " << srcReg << endl;
        } else {
            int offset = -(paraOrder - 4) * 4 - 52;
            mipsFile << "addi $k0 $fp " << offset << endl; // 计算参数相对于fp的基地址
        ...
        mipsFile << "sw " << srcReg << " 0($k0)" << endl; // 将变量的值写回存储器
    } else if (fourComExpr.target.at(0) == '$') {
        // 直接对寄存器赋值
...
void MipsGenerator::genExprCode(FourComExpr &fourComExpr) {
    ...
    // 第3步 :: 根据目标值进行计算
    if (isdigit(srcReg1[0])) {
        mipsFile << "li $k0 " << srcReg1 << endl;
        srcReg1 = "$k0";
    }

    string dstReg;
    if (varToTempReg.find(fourComExpr.target) != varToTempReg.end()) {
        dstReg = varToTempReg.find(fourComExpr.target)->second;
    } else {
        dstReg = fourComExpr.target;
    }
    if (fourComExpr.op == "+") {
        if (isdigit(srcReg2[0])) {
        ...
        if (isdigit(srcReg2[0])) {
            mipsFile << "subi " << dstReg << " " << srcReg1 << " " << srcReg2 << endl;
        } else {
...
void MipsGenerator::keepScene() { // 保存现场
    // 保存函数的参数 --- 用到哪些就得保存哪些
...
void MipsGenerator::initialStack(const string &funcName) {
    ...
    mipsFile << "addi $sp,$sp,-48" << endl; // 移动栈指针 --- 保存现场需要 48个字节的栈空间
    vector<SymbolItem> paraVec;
    if (funcName != "main") {
        paraVec = symbolTabel.getSymbol(funcName).getParaVec(); // 得到函数的参数表
    }
    int funcMemSize = 0; // 多余的函数参数所需空间
    if (paraVec.size() > 4) { // 我的main函数是没有压栈的
        funcMemSize += ((int) paraVec.size() - 4) * 4;
    }
    ...
    mipsFile << "addi $sp,$sp,-4" << endl; // 应该多-4
...
void MipsGenerator::genReturnCode(FourComExpr &fourComExpr) {
    ...
    if (codeScope == "main") {
        ...
        if (srcReg != "$v0") mipsFile << "move $v0 " << srcReg << endl;
    }
...
void MipsGenerator::genFuncRefCode(FourComExpr &fourComExpr) {
    ...
    for (auto &it : fourComExpr.valueParaTab) {  // 先把参数保存到a0~a3中
        if (index < 4) {
            ...
            if (srcReg != dstReg) mipsFile << "move " << dstReg << " " << srcReg << endl;
        } else {
            // 往寄存器空间之上存
            string srcReg = getEleValue(it.name, "$k0", it.index); // 保存到$k0中
            int offset = (index - 4) * 4;
            offset = 0 - offset - 56; // 这里要保存到即将进入的栈空间当中
...
string MipsGenerator::getLocalAddr(const string &varName, int offset, const string &dstReg) {
    if (codeScope == "complete_flower_num" && varToTempReg.find(varName) != varToTempReg.end()) {
        return varToTempReg.find(varName)->second; // 返回全局寄存器
    }
...
string MipsGenerator::getTempReg(const string &tempReg, const string &dstReg) {
    // tempReg : $t1
    // mipsFile << "move " << dstReg << " " << tempReg << endl;
    return tempReg;
}
...
string MipsGenerator::getParaValue(int paraOrder, const string &dstReg) {
    if (paraOrder < 4) {
        string srcReg = "$a" + to_string(paraOrder); // 保存参数的寄存器
        // mipsFile << "move " << dstReg << " " << srcReg << endl; // 放到目标寄存器中
        return srcReg;
    } else {
        int offset = -(paraOrder - 4) * 4 - 52;
...
void MipsGenerator::genBranchCode(FourComExpr &fourComExpr) {
    ...
    string srcReg = getTempReg(fourComExpr.left, "$k0");
    output.append(srcReg); // 记得跳转语句这个地方只能是寄存器
